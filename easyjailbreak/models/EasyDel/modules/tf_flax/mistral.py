...
class FlaxMistralAttention(nn.Module):
    ...
    def _split_kv_heads(self, hidden_states):
        return hidden_states.reshape(hidden_states.shape[:2] + (self.num_key_value_heads, self.head_dim))
    ...
    def __call__(
        ...
        attn_weights = nn.dot_product_attention(query=query, key=key, value=value, bias=attention_bias,
                                               dtype=jnp.promote_types(self.dtype, jnp.float32))
        attn_weights = attn_weights.astype(key.dtype)
        # attn_output = self._merge_heads(attn_output)
        attn_output = self.o_proj(attn_weights.reshape(batch_size, hidden_states.shape[1], self.hidden_size))
...
append_call_sample_docstring(FlaxMistralModel, _CHECKPOINT_FOR_DOC, FlaxBaseModelOutput, _CONFIG_FOR_DOC)
...
append_call_sample_docstring(FlaxMistralForCausalLM, _CHECKPOINT_FOR_DOC, FlaxCausalLMOutput, _CONFIG_FOR_DOC)
...
class FlaxMistralForSequenceClassificationModule(nn.Module):
    ...
    def setup(self):
        self.model = FlaxMistralModule(self.config, dtype=self.dtype, param_dtype=self.param_dtype)
        self.score = nn.Dense(
