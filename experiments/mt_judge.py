...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--judge-file",
        type=str,
        default="fastchat/llm_judge/data/judge_prompts.jsonl",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Amodel to be evaluated",
    )
    parser.add_argument(
        "--parallel", type=int, default=1, help="The number of concurrent API calls."
    )
    parser.add_argument(
        "--filename", type=str, default="MT-Bench.jsonl", help="MT-Bench.jsonl filename"
    )
    parser.add_argument(
        "--first-n", type=int, help="A debug option. Only run the first `n` judgments."
    )
    # parser.add_argument(
    #     "-y", type=bool, help="skip confirm.", default=False
    # )
    args = parser.parse_args()

    question_file = f"fastchat/llm_judge/data/{args.bench_name}/question.jsonl"
    # answer_dir = f"fastchat/llm_judge/data/{args.bench_name}/model_answer"
    ref_answer_dir = f"fastchat/llm_judge/data/{args.bench_name}/reference_answer"

    # Load questions
    questions = load_questions(question_file, None, None)

    # Load answers
    answer_file = f"outputs/{args.model}/{args.filename}"
    model_answers = {}
    with jsonlines.open(answer_file) as f:
        answers = {}
        for item in f:
            answers[item["question_id"]] = item
        model_answers[args.model] = answers
    ...
    if args.mode == "single":
        ...
        output_file = (
            f"fastchat/llm_judge/data/{args.bench_name}/model_judgment/{args.judge_model}/{args.model}_single.jsonl"
        )
        ...
        output_file = (
            f"fastchat/llm_judge/data/{args.bench_name}/model_judgment/{args.judge_model}/{args.model}_pair.jsonl"
        )
